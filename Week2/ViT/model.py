#tutorial: https://github.com/s-chh/PyTorch-Scratch-Vision-Transformer-ViT/blob/main/model.py
...
class EmbedLayer(nn.Module):
    ...
    Creates a patch embedding from the image using a Conv2D operation.
    Adds a learnable positional embedding vector to give spaitial properties to patch embeddings.
    Adds a classification token, which is used to classify the image.
    ...
    @input: 
        x(tensor) : image tensor of shape (batch size, number of input channels, image width, image height)

    @returns:
        Tensor : embedding of the image of shape (batch size, sequence length, embedding dimension)
    """

    def __init__(self, n_channels, embed_dim, image_size, patch_size, dropout=0.0):
        super().__init__()
        self.conv = nn.Conv2d(n_channels, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class SelfAttention(nn.Module):
    ...
    @returns:
    Tensor : output after self attention module of shape (batch size, sequence length, embedding dimension)
    """
